...
try {
  ...
  await a.evaluate(`(() => { document.querySelector('#lobbyChat [data-act=room-chat-toggle]').click(); const input = document.getElementById('roomChatInput'); input.value = '대기방 안녕!'; document.querySelector('#lobbyChat [data-act=room-chat-send]').click() })()`)
  await waitFor(b, `Room.current.messages.some((message) => message.text === '대기방 안녕!')`)
  await a.evaluate(`document.getElementById('roomCode').click()`)
  const lobbyChatClosed = await a.evaluate(`!Room.current.chatOpen && document.querySelector('#lobbyChat .chat-panel') === null`)
  if (!lobbyChatClosed) throw new Error('대기방 채팅 바깥을 눌러도 닫히지 않습니다')
  ok('대기방 전체 채팅 · 채팅 바깥 터치로 닫기')
  ...
  if (!firstSurvivors.alive.includes(setterPids.C) || !firstSurvivors.alive.includes(setterPids.D) || !firstSurvivors.eliminated.includes(setterPids.A) || firstSurvivors.eliminated.length !== 2 || !firstSurvivors.next || !firstSurvivors.host || !firstSurvivors.stillInRoom) throw new Error('대장전 방장 탈락 후 방 유지 실패: ' + JSON.stringify(firstSurvivors))
  ...
  const eliminatedWatching = await Promise.all([a, b].map((player) => player.evaluate(`Room.current.me.role === 'spectator' && document.body.classList.contains('watching')`)))
  ...
  if (champion.pid !== setterPids.C || !champion.title.includes('최후의 대장')) throw new Error('대장전 최종 우승 처리 실패: ' + JSON.stringify(champion))
  ok('대장전 → 방장 탈락 후 방장·관전 유지 · 다음 판 진행 · 마지막 1명 우승')
