...
try {
  ...
  await Promise.all(four.map((player) => waitFor(player, `Room.current.over && Room.current.tournamentAlive.length === 2 && document.querySelector('.tournament-tree')`, 12000)))
  const firstBracket = await a.evaluate(`({ alive: Room.current.tournamentAlive, eliminated: Room.current.tournamentEliminated, rounds: Room.current.tournamentRounds.length, nodes: document.querySelectorAll('.tournament-tree-node').length, advances: document.querySelectorAll('.tournament-tree-node.advance').length, faded: document.querySelectorAll('.tournament-tree-node.eliminated').length, pending: document.querySelectorAll('.tournament-tree-node.pending').length, lines: document.querySelectorAll('.tournament-tree path').length, manualNext: !!document.querySelector('[data-act=room-next]'), autoWithinTen: Room.current.tournamentNextAt - Date.now() <= 10000 })`)
  if (firstBracket.alive.slice().sort().join(',') !== firstAdvances.slice().sort().join(',') || firstBracket.eliminated.length !== 2 || firstBracket.rounds !== 1 || firstBracket.nodes !== 7 || firstBracket.advances !== 2 || firstBracket.faded !== 2 || firstBracket.pending !== 3 || firstBracket.lines !== 6 || firstBracket.manualNext || !firstBracket.autoWithinTen) throw new Error('토너먼트 아래→위 트리·자동 진행 처리 실패: ' + JSON.stringify(firstBracket))
  ...
  const finalTree = await a.evaluate(`({ rounds: Room.current.tournamentRounds.length, nodes: document.querySelectorAll('.tournament-tree-node').length, champion: document.querySelectorAll('.tournament-tree-node.champion').length, faded: document.querySelectorAll('.tournament-tree-node.eliminated').length })`)
  if (finalTree.rounds !== 2 || finalTree.nodes !== 7 || finalTree.champion !== 1 || finalTree.faded !== 3) throw new Error('토너먼트 전체 라운드 누적 트리 실패: ' + JSON.stringify(finalTree))
  ok('토너먼트 4인 → 아래 참가자에서 위 우승자까지 누적 트리 · 연결선 · 진출/탈락 애니메이션 · 최종 우승')

  await createMode('tournament', 4)
  await a.evaluate(`document.querySelector('[data-act=room-start]').click()`)
  await Promise.all([a, b].map((player) => waitFor(player, `TW.state?.mode === 'tournament' && Room.current.tournamentPairs.length === 1`, 12000)))
  const twoFinalist = await a.evaluate(`Room.current.tournamentPairs[0][0]`)
  const twoByPid = new Map([[await a.evaluate(`Room.current.me.pid`), a], [await b.evaluate(`Room.current.me.pid`), b]])
  await twoByPid.get(twoFinalist).evaluate(`(async () => { for (const jamo of TW.state.answerJamo) TW.press(jamo); await TW.submit() })()`)
  await Promise.all([a, b].map((player) => waitFor(player, `Room.current.over && Room.current.tournamentChampion && document.querySelector('.tournament-tree-node.champion')`, 12000)))
  const twoTree = await a.evaluate(`({ nodes: document.querySelectorAll('.tournament-tree-node').length, lines: document.querySelectorAll('.tournament-tree path').length, faded: document.querySelectorAll('.tournament-tree-node.eliminated').length })`)
  if (twoTree.nodes !== 3 || twoTree.lines !== 2 || twoTree.faded !== 1) throw new Error('2인 결승 아래→위 대진표 실패: ' + JSON.stringify(twoTree))
  ok('토너먼트 2인 → 첫 라운드가 결승이어도 아래→위 한 경기 대진표와 우승자 표시')
