...
try {
  ...
  if (groupedModes.solo.join(',') !== '같은 단어 대결,출제 대결,연판,서바이벌,토너먼트' || groupedModes.team.join(',') !== '협동,팀전,팀 출제 대결,스파이전') throw new Error('개인전/팀전 모드 분류 실패: ' + JSON.stringify(groupedModes))
  ...
  async function createMode(kind, size = 4, rounds = 1) {
    ...
    await a.evaluate(`(() => {
      ...
      document.querySelector('[data-room-category="${['versus', 'setter', 'relay', 'captain', 'tournament'].includes(kind) ? 'solo' : 'team'}"]').click()
  ...
  for (const pid of firstAdvances) await fourByPid.get(pid).evaluate(`(async () => { for (const jamo of TW.state.answerJamo) TW.press(jamo); await TW.submit() })()`)
  await waitFor(a, `Room.current.results.size === 2`, 12000)
  await a.evaluate(`(() => { document.getElementById('btnMenu').click(); document.querySelector('[data-act=room-force]').click() })()`)
  await Promise.all(four.map((player) => waitFor(player, `Room.current.over && Room.current.tournamentAlive.length === 2 && document.querySelector('.tournament-score')`, 12000)))
  const firstBracket = await a.evaluate(`({ alive: Room.current.tournamentAlive, eliminated: Room.current.tournamentEliminated, next: !!document.querySelector('[data-act=room-next]') })`)
  if (firstBracket.alive.slice().sort().join(',') !== firstAdvances.slice().sort().join(',') || firstBracket.eliminated.length !== 2 || !firstBracket.next) throw new Error('토너먼트 1라운드 진출 처리 실패: ' + JSON.stringify(firstBracket))
  await a.evaluate(`document.querySelector('[data-act=room-next]').click()`)
  ...
  ok('토너먼트 4인 → 1대1 상대 화면 · 라운드별 진출/탈락 · 최종 우승')
