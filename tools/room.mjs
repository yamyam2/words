...
try {
  ...
  if (groupedModes.solo.join(',') !== '같은 단어 대결,출제 대결,연판,서바이벌,토너먼트,원탁 추리' || groupedModes.team.join(',') !== '협동,팀전,팀 출제 대결,스파이전') throw new Error('개인전/팀전 모드 분류 실패: ' + JSON.stringify(groupedModes))
  ...
  await a.evaluate(`document.querySelector('[data-act=room-start]').click()`)
  await Promise.all([a, b].map((player) => waitFor(player, `Room.current?.kind === 'roundtable' && !document.getElementById('roundTable').hidden && Room.current.roundtableSeats.length === 2`, 12000)))
  ...
  await firstTurn.evaluate(`(() => { const input = document.getElementById('roundtableClue'); input.value = Words.answers[TW.state.size].find((word) => word !== TW.state.answer); input.form.requestSubmit() })()`)
  ...
  const challengerPid = await a.evaluate(`Room.current.turnPid`); const challenger = roundtableByPid.get(challengerPid)
  await challenger.evaluate(`(() => { document.querySelector('[data-roundtable-challenge-open]').click(); const input = document.getElementById('roundtableAnswer'); input.value = Words.answers[TW.state.size].find((word) => word !== TW.state.answer); input.form.requestSubmit() })()`)
  await Promise.all([a, b].map((player) => waitFor(player, `Room.current.roundtableLocked.has('${challengerPid}')`, 12000)))
  if (await a.evaluate(`Room.current.roundtableRows.length !== 1`)) throw new Error('원탁 추리 비공개 정답 도전이 색 힌트 기록에 노출됐습니다')
  await challenger.evaluate(`(() => { const input = document.getElementById('roundtableClue'); input.value = Words.answers[TW.state.size].find((word) => word !== TW.state.answer); input.form.requestSubmit() })()`)
  await Promise.all([a, b].map((player) => waitFor(player, `Room.current.roundtableRows.length === 2 && !Room.current.roundtableLocked.has('${challengerPid}')`, 12000)))
  const solver = roundtableByPid.get(await a.evaluate(`Room.current.turnPid`))
  await solver.evaluate(`(() => { document.querySelector('[data-roundtable-challenge-open]').click(); const input = document.getElementById('roundtableAnswer'); input.value = TW.state.answer; input.form.requestSubmit() })()`)
  ...
  if (roundtableResult.seats !== 2 || roundtableResult.history !== 2 || roundtableResult.answer !== await a.evaluate(`TW.state.answer`) || !roundtableResult.crown) throw new Error('원탁 추리 결과 처리 실패: ' + JSON.stringify(roundtableResult))
  ok('원탁 추리 → 무작위 원탁 좌석 · 공개 색 힌트 · 비공개 정답 도전 · 실패 잠금/다음 차례 해제 · 즉시 우승')
