...
;(function () {
  ...
  TW.SHEETS.scoreboard = () => {
    ...
    if (room.kind === 'coop') {
      ...
      const detail = soloWin ? '마지막 기회에서 정답을 맞혔어요' : winner ? `정답자 ${winnerCount}명 · 빠른 순서` : won ? `${room.coopResult.tries}/${room.maxTries}번 만에 성공` : `정답 · ${room.size}칸`
      let rank = 0
  ...
  function rematchControls() {
    const voters = room?.readyVoters?.size ? room.readyVoters : new Set(onlinePlayers().map((p) => p.pid))
  ...
  function processReady(payload) {
    ...
    const pid = String(payload.pid || '')
    if (!room.peers.get(pid)?.online) return
    room.readyPids.add(pid); publishReadyState()
  }
